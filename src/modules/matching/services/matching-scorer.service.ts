...
export class MatchingScorerService {
    ...
    public static async calculateSemanticScore(
        candidate: CandidateMatchingProfile,
        job: JobMatchingRequirements,
     ...
     */
    public static blendFinalScore(
        deterministicScore: number,
        semanticResult: SemanticScoreResult | null
    ): number {
        ...
        const blended =
            deterministicScore * MATCHING_THRESHOLDS.DETERMINISTIC_BLEND_WEIGHT +
    ...
    private static evaluateSkills(
        ...
        const candidateSkillKeys = new Set(
            candidate.skills.map((s) => normalizeSkillLookupKey(s.name)).filter(Boolean)
        );
    ...
    private static evaluateEducation(
        candidate: CandidateMatchingProfile,
        _job: JobMatchingRequirements
    ): MatchingFactors["education"] {
        let score = 5; 
    ...
    private static parseAiResponse(raw: string): { semanticScore?: number; reasoning?: string } | null {
        try {
