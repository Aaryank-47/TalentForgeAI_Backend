...
describe("MatchingScorerService Unit Tests", () => {
    const mockCandidate: CandidateMatchingProfile = {
        id: "cand_1",
        userId: "user_cand_1",
    ...
    describe("LEVEL 1: Deterministic Scoring", () => {
        it("should calculate high deterministic score for a well-matched candidate", () => {
            ...
            expect(result.factors.skills.requiredMatched).toEqual(
        ...
        it("should penalize score when candidate is missing required skills", () => {
            const candidateWithoutSkills: CandidateMatchingProfile = {
                ...mockCandidate,
                skills: [{ name: "Python", yearsOfExperience: 2 }]
            };

            const result = MatchingScorerService.calculateDeterministicScore(
                candidateWithoutSkills,
                mockJob
            );

            expect(result.score).toBeLessThanOrEqual(60);
            expect(result.factors.skills.requiredMissing).toEqual(
                expect.arrayContaining(["React", "Node.js", "TypeScript"])
            );
        ...
        it("should handle experience boundaries (underqualified candidate)", () => {
            const juniorCandidate: CandidateMatchingProfile = {
                ...mockCandidate,
                totalExperience: 1.0,
                skills: []
            };
        ...
        it("should award full location points for REMOTE jobs regardless of candidate location", () => {
            const candidateRemote: CandidateMatchingProfile = {
                ...mockCandidate,
        ...
        it("should evaluate onsite location match correctly", () => {
            const onsiteJob: JobMatchingRequirements = {
                ...mockJob,
                workplaceType: "ONSITE",
                location: "San Francisco, CA"
            };

            const resultMatch = MatchingScorerService.calculateDeterministicScore(
                mockCandidate,
                onsiteJob
            );
            expect(resultMatch.factors.location.score).toBe(15);

            const nonLocalCandidate: CandidateMatchingProfile = {
                ...mockCandidate,
                currentLocation: "London, UK",
                preferredLocation: null
            };

            const resultMismatch = MatchingScorerService.calculateDeterministicScore(
                nonLocalCandidate,
                onsiteJob
            );
    ...
    describe("LEVEL 2: Optional Semantic / AI Scoring", () => {
        ...
        it("should invoke AI evaluation when deterministic score >= 70 and blend score", async () => {
            const spyGenerateText = jest.spyOn(OpenRouterClient, "generateText").mockResolvedValue(
                JSON.stringify({
                    semanticScore: 92,
            ...
            const finalBlended = MatchingScorerService.blendFinalScore(
                deterministicScore,
                semanticResult
            );
        ...
        it("should gracefully fallback to deterministic score if AI throws error", async () => {
            const spyGenerateText = jest.spyOn(OpenRouterClient, "generateText").mockRejectedValue(
                new Error("OpenRouter rate limit or timeout")
            );
