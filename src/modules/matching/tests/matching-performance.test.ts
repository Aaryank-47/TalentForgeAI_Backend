...
describe("Matching Performance & Scalability Diagnostics", () => {
    it("should process targeted candidate subset without Cartesian product scan across full candidate base", async () => {
        ...
        const mockFilteredCandidates: CandidateMatchingProfile[] = Array.from(
            { length: 100 },
            (_, i) => ({
                id: `cand_${i + 1}`,
                userId: `user_${i + 1}`,
        ...
        let aiCallCount = 0;
        jest.spyOn(MatchingScorerService, "calculateSemanticScore").mockImplementation(
            async (_cand, _job, score) => {
                aiCallCount++;
                return {
                    score: score + 5,
                    reasoning: "Good fit",
