...
matchingRoutes.post(
    "/candidate/recalculate-matches",
    authMiddleware,
    ensureCandidateProfile,
    MatchingController.recalculateCandidateMatches
);

// ── Recruiter / Employer Endpoints ──────────────────────────────────────────
matchingRoutes.get(
    "/recruiter/jobs/:jobId/matched-candidates",
    authMiddleware,
...
matchingRoutes.post(
    "/recruiter/jobs/:jobId/recalculate-matches",
    authMiddleware,
    authorize("EMPLOYER", "ADMIN", "SUPER_ADMIN"),
    MatchingController.recalculateJobMatches
);
