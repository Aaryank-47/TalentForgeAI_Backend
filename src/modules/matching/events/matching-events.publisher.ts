...
export class MatchingEventsPublisher {
     ...
     */
    public static async onCandidateMatchingDataChanged(
        candidateId: string,
        changedFields: string[] | Record<string, any>
    ): Promise<boolean> {
        const fieldNames = Array.isArray(changedFields)
            ? changedFields
            : Object.keys(changedFields);

        const isRelevant = fieldNames.some((field) =>
            MATCHING_RELEVANT_CANDIDATE_FIELDS.has(field)
        );
     ...
     */
    public static async onJobMatchingDataChanged(
        jobId: string,
        changedFields: string[] | Record<string, any>
    ): Promise<boolean> {
        const fieldNames = Array.isArray(changedFields)
        ...
        const isRelevant = fieldNames.some((field) =>
            MATCHING_RELEVANT_JOB_FIELDS.has(field)
        );
