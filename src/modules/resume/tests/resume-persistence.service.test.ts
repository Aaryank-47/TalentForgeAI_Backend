...
import { ResumePersistenceService, type ResumePersistencePort } from "../services/resume-persistence.service.js";

describe("ResumePersistenceService", () => {
    const data: ResumeParsingResult = {
        personal: {
            fullName: "Jane Doe",
    ...
    it("delegates normalized data and returns the persistence result", async () => {
        ...
        const persist = jest.fn<ResumePersistencePort["persist"]>().mockResolvedValue(result);
    ...
    it("wraps persistence failures without exposing parsed resume data", async () => {
        const persist = jest.fn<ResumePersistencePort["persist"]>().mockRejectedValue(new Error("database unavailable"));
        const service = new ResumePersistenceService({ persist });
    ...
    it("preserves existing application errors", async () => {
        ...
        const persist = jest.fn<ResumePersistencePort["persist"]>().mockRejectedValue(notFound);
        const service = new ResumePersistenceService({ persist });
    ...
    it("rejects a blank candidate ID before touching the repository", async () => {
        const persist = jest.fn<ResumePersistencePort["persist"]>();
        const service = new ResumePersistenceService({ persist });
