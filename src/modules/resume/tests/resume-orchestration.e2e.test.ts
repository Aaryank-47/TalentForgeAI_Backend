...
describe("Resume Processing Module — Complete Orchestration & E2E Scenarios (A through J)", () => {
    ...
    test("Scenario A: Successful DOCX resume -> Extraction -> AI -> Normalization -> Persistence -> COMPLETED -> Socket events", async () => {
        ...
        jest.spyOn(prisma.resume as any, "findUnique").mockImplementation(async (args: any) => {
            if (args.where.id === resumeId) return resumeDbRecord;
            return null;
        });
        jest.spyOn(prisma.candidate as any, "findUnique").mockImplementation(async (args: any) => {
            if (args.where.userId === mockCandidateA.id) return { id: mockCandidateA.id };
            return null;
        });
        ...
        const mockFileFetcher = jest.fn<(url: string) => Promise<Buffer>>().mockResolvedValue(Buffer.from("mock-docx-bytes"));
        ...
        const mockParserService = {
            parseResumeDocument: jest.fn<any>(),
            parseResumeText: jest.fn<any>().mockResolvedValue({
        ...
        client.on(RESUME_SOCKET_EVENTS.STAGE_CHANGE, (payload: ResumeStageEventPayload) => {
    ...
    test("Scenario B: Successful PDF direct AI parsing -> Direct AI -> Normalization -> Persistence -> COMPLETED", async () => {
        ...
        const mockFileFetcher = jest.fn<(url: string) => Promise<Buffer>>().mockResolvedValue(Buffer.from("%PDF-1.4"));
        const mockDocExtractor = { extractDocument: jest.fn<any>() };
        const mockParserService = {
    ...
    test("Scenario C: PDF direct AI parsing failure -> local extraction fallback -> successful parsing with mode: FALLBACK", async () => {
        ...
        const mockParserService = {
            parseResumeDocument: jest.fn<any>().mockRejectedValue(new Error("Direct PDF document parsing failed")),
            parseResumeText: jest.fn<any>().mockResolvedValue({
                personal: { fullName: "Jane Doe", email: "jane@talentforge.ai" },
    ...
    test("Scenario D: Permanent AI failure -> immediately marks FAILED in DB and emits Socket.IO FAILED event without retrying", async () => {
        ...
        const mockDocExtractor = { extractDocument: jest.fn<any>() };
        const mockParserService = {
            parseResumeDocument: jest.fn<any>().mockRejectedValue(new OpenRouterError("Unauthorized: invalid API key", 401)),
            parseResumeText: jest.fn<any>()
        };
    ...
    test("Scenario E: Transient AI failure -> rethrows for BullMQ retry on attempt 1, does NOT emit FAILED event yet", async () => {
        ...
        const mockFileFetcher = jest.fn<(url: string) => Promise<Buffer>>().mockResolvedValue(Buffer.from("%PDF-1.4"));
        const mockDocExtractor = {
            extractDocument: jest.fn<any>().mockResolvedValue({ text: "Sample text", wordCount: 2 })
        };
        const mockParserService = {
            parseResumeDocument: jest.fn<any>().mockRejectedValue(new Error("Direct failed")),
            parseResumeText: jest.fn<any>().mockRejectedValue(new Error("Network connection reset by peer"))
        };
    ...
    test("Scenario F: Invalid / unsupported document -> Permanent failure classification", async () => {
        ...
        const mockFileFetcher = jest.fn<(url: string) => Promise<Buffer>>().mockResolvedValue(Buffer.from("invalid-binary"));
        const mockDocExtractor = {
            extractDocument: jest.fn<any>().mockRejectedValue(new UnsupportedFileTypeError("application/x-msdownload"))
        };
        const mockParserService = {
            parseResumeDocument: jest.fn<any>().mockRejectedValue(new UnsupportedFileTypeError("application/x-msdownload")),
            parseResumeText: jest.fn<any>()
        };
    ...
    test("Scenario G: Persistence / database failure -> Socket COMPLETED is NOT emitted and error propagates to BullMQ", async () => {
        ...
        const mockPersistenceService = {
            persistResumeData: jest.fn<any>().mockRejectedValue(new Error("Database transaction rolled back"))
        };
    ...
    test("Scenario H: Duplicate / repeated processing -> skips pipeline execution if already COMPLETED", async () => {
        ...
        const pipeline = { execute: mockExecute } as any;
        const worker = new ResumeProcessingWorker(pipeline);
    ...
    test("Scenario I: Socket.IO progress delivery verifies structured stage change events and timing", async () => {
        ...
        jest.spyOn(prisma.resume as any, "findUnique").mockResolvedValue({
            id: resumeId,
            candidateId: mockCandidateA.id,
            parsingStatus: "QUEUED",
            parsingStartedAt: null,
        ...
        jest.spyOn(prisma.candidate as any, "findUnique").mockResolvedValue({ id: mockCandidateA.id });
        ...
        const stagePayloads: ResumeStageEventPayload[] = [];
    ...
    test("Scenario J: Unauthorized resume Socket.IO subscription -> rejects Candidate B from Candidate A's resume room", async () => {
        ...
        jest.spyOn(prisma.candidate as any, "findUnique").mockImplementation(async (args: any) => {
            if (args.where.userId === mockCandidateB.id) return { id: mockCandidateB.id };
            return null;
        });
