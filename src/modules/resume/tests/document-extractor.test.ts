...
describe("Document Extractor Unit Tests", () => {
  ...
  describe("PDF Extraction (extractPdf)", () => {
    ...
    it("3. Empty PDF buffer throws DocumentExtractionError", async () => {
      await expect(documentExtractorService.extractPdf(Buffer.from(""))).rejects.toThrow(
        DocumentExtractionError
      );
    });

    it("5. Corrupted PDF throws DocumentExtractionError", async () => {
      await expect(documentExtractorService.extractPdf(Buffer.from("corrupted pdf data"))).rejects.toThrow(
        DocumentExtractionError
      );
    });

    it("11. Scanned PDF detection throws ScannedPdfDetectedError when pageCount > 0 but text is empty", async () => {
      await expect(documentExtractorService.extractPdf(scannedPdfBuffer)).rejects.toThrow(
        ScannedPdfDetectedError
      );
    });
  });
  ...
  describe("DOCX Extraction (extractDocx)", () => {
    ...
    it("4. Empty DOCX buffer throws DocumentExtractionError", async () => {
      await expect(documentExtractorService.extractDocx(Buffer.from(""))).rejects.toThrow(
        DocumentExtractionError
      );
    });
    ...
    it("6. Corrupted DOCX parsing failure throws DocumentExtractionError", async () => {
      ...
      await expect(documentExtractorService.extractDocx(Buffer.from("corrupted docx"))).rejects.toThrow(
        DocumentExtractionError
      );
    ...
    it("10. Empty extracted DOCX throws EmptyDocumentTextError", async () => {
      ...
      await expect(documentExtractorService.extractDocx(Buffer.from("empty docx"))).rejects.toThrow(
        EmptyDocumentTextError
      );
  ...
  describe("DocumentExtractorService Dispatcher (extractDocument)", () => {
    ...
    it("8. MIME type normalization routes DOCX correctly", async () => {
      ...
      const result = await documentExtractorService.extractDocument(
        Buffer.from("docx"),
    ...
    it("7. Unsupported MIME type throws UnsupportedFileTypeError", async () => {
      await expect(
        documentExtractorService.extractDocument(Buffer.from("txt"), "text/plain")
