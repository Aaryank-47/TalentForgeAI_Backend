 ...
 */
export function normalizeSkillLookupKey(raw: string): string {
    if (!raw) return "";
    return raw
        .trim()
        .toLowerCase()
 ...
 */
export function normalizeLookupKey(val: string | null | undefined): string {
    if (!val) return "";
    return val
        .trim()
        .toLowerCase()
 ...
 */
export function normalizeText(val: string | null | undefined): string | null {
    if (val === null || val === undefined) return null;
    const cleaned = val
        .replace(/\r\n/g, "\n")
        .replace(/\r/g, "\n")
 ...
 */
export function normalizeEmploymentType(val: EmploymentType | string | null | undefined): EmploymentType | null {
    if (!val) return null;

    if (Object.values(EmploymentType).includes(val as EmploymentType)) {
