...
import type {
    ...
    ResumeSubscribePayload,
    ResumeSubscribedResponsePayload,
    ResumeUnsubscribePayload
} from "./resume-socket.types.js";


// Registers client event listeners for the authenticated resume socket.
export function registerResumeSocketHandlers(socket: Socket): void {
    const user = socket.data?.user || socket.user;

    if (!user || !user.id) {
        logger.warn(
            { socketId: socket.id },
            "[ResumeSocketHandler] Socket connected without user context. Disconnecting..."
        );
    ...
    socket.on(RESUME_SOCKET_EVENTS.SUBSCRIBE, async (data: ResumeSubscribePayload) => {
        try {
            if (!data || !data.resumeId || typeof data.resumeId !== "string") {
                const errorPayload: ResumeSocketErrorPayload = {
            ...
            const subscribedPayload: ResumeSubscribedResponsePayload = {
                resumeId,
    ...
    socket.on(RESUME_SOCKET_EVENTS.UNSUBSCRIBE, async (data: ResumeUnsubscribePayload) => {
        try {
            ...
            logger.warn(
                ...
                "[ResumeSocketHandler] Error handling resume unsubscribe"
            );
        }
    });
