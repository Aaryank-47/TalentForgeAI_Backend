...
export class ResumeProgressPublisher {
    private static namespaceInstance: Namespace | null = null;

    // Attaches the Socket.IO namespace to the publisher.
    public static setNamespace(namespace: Namespace): void {
        this.namespaceInstance = namespace;
    }

    // Gets the currently registered namespace instance.
    public static getNamespace(): Namespace | null {
     ...
     */
    public static async publishStageChange(
        ...
        try {
            ...
            if (!this.namespaceInstance) {
                logger.debug(
                    { resumeId, stage },
                    "[ResumeProgressPublisher] Socket namespace not initialized; skipping live broadcast"
                );
                return;
            }

            const stagePayload: ResumeStageEventPayload = {
                resumeId,
                jobId,
                candidateId,
                stage,
     ...
     * Note: NEVER throws.
     */
    public static async publishCompleted(meta: StageChangeMeta): Promise<void> {
        try {
            ...
            if (!this.namespaceInstance) {
                logger.debug(
                    { resumeId },
                    "[ResumeProgressPublisher] Socket namespace not initialized; skipping COMPLETED broadcast"
                );
                return;
            }

            const completedPayload: ResumeCompletedEventPayload = {
                resumeId,
                jobId,
                candidateId,
                stage: "COMPLETED",
     ...
     * Note: NEVER throws.
     */
    public static async publishFinalFailure(
        meta: StageChangeMeta,
        errorMessage: string
    ): Promise<void> {
        try {
            ...
            const failedPayload: ResumeFailedEventPayload = {
                resumeId,
                jobId,
                candidateId,
                stage: "FAILED",
