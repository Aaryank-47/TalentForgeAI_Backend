...
const fingerprint = (...parts: Array<string | null>): string =>
    parts.map((part) => (part ?? "").trim().toLocaleLowerCase()).join("|");
...
export class ResumePersistenceRepository {
    ...
    private async persistInTransaction(
        ...
        candidateId: string,
        data: ResumeParsingResult
    ): Promise<ResumePersistenceResult> {
        ...
        if (personal.fullName != null) candidateUpdate.fullName = personal.fullName;
        if (personal.phoneNumber != null) candidateUpdate.phoneNumber = personal.phoneNumber;
        if (personal.currentLocation != null) candidateUpdate.currentLocation = personal.currentLocation;
        if (personal.linkedinUrl != null) candidateUpdate.linkedinUrl = personal.linkedinUrl;
        if (personal.githubUrl != null) candidateUpdate.githubUrl = personal.githubUrl;
        if (personal.portfolioUrl != null) candidateUpdate.portfolioUrl = personal.portfolioUrl;
        if (personal.websiteUrl != null) candidateUpdate.websiteUrl = personal.websiteUrl;
        if (professional.headline != null) candidateUpdate.headline = professional.headline;
        if (professional.bio != null) candidateUpdate.bio = professional.bio;
        if (professional.currentCompany != null) candidateUpdate.currentCompany = professional.currentCompany;
        if (professional.currentDesignation != null) candidateUpdate.currentDesignation = professional.currentDesignation;
    ...
    private async persistSkills(
        tx: TransactionClient,
        candidateId: string,
        skills: ResumeParsingResult["skills"]
    ): Promise<Pick<ResumePersistenceResult, "skillsCreated" | "skillsUpdated">> {
        ...
        const incoming = new Map<string, { name: string; yearsOfExperience: number | null }>();
        ...
        const changedSkills = [] as Array<{ id: string; candidateId: string; name: string; yearsOfExperience: number | null; skillId: string | null }>;
        for (const [key, skill] of incoming) {
            const current = existingByKey.get(key);
            const skillId = canonicalByKey.get(skill.name.trim().toLocaleLowerCase())?.id ?? null;
            if (!current || (current.name === skill.name && current.yearsOfExperience === skill.yearsOfExperience && current.skillId === skillId)) continue;
    ...
    private async persistExperiences(
        tx: TransactionClient,
        candidateId: string,
        experiences: ResumeParsingResult["experience"]
    ): Promise<Pick<ResumePersistenceResult, "experiencesCreated" | "experiencesUpdated">> {
        ...
        const incoming = new Map<string, ExperienceValues>();

        for (const experience of experiences) {
            const startDate = parseDate(experience.startDate);
            const endDate = parseDate(experience.endDate);
            if (!startDate || !experience.employmentType || (!experience.currentlyWorking && !endDate)) continue;
        ...
        const changedExperiences = [] as Array<ExperienceValues & { id: string; candidateId: string }>;
        for (const [key, experience] of incoming) {
            ...
            if (!unchanged) changedExperiences.push({ id: current.id, candidateId, ...experience });
        }
    ...
    private async persistEducation(
        tx: TransactionClient,
        candidateId: string,
        education: ResumeParsingResult["education"]
    ): Promise<Pick<ResumePersistenceResult, "educationCreated" | "educationUpdated">> {
        ...
        const incoming = new Map<string, EducationValues>();

        for (const item of education) {
            const startDate = parseDate(item.startDate);
            const endDate = parseDate(item.endDate);
            if (!startDate || !item.gradingSystem || (!item.currentlyStudying && !endDate)) continue;
        ...
        const changedEducation = [] as Array<EducationValues & { id: string; candidateId: string }>;
        for (const [key, item] of incoming) {
            ...
            if (!unchanged) changedEducation.push({ id: current.id, candidateId, ...item });
        }
    ...
    private async persistProjects(
        tx: TransactionClient,
        candidateId: string,
        projects: ResumeParsingResult["projects"]
    ): Promise<Pick<ResumePersistenceResult, "projectsCreated" | "projectsUpdated">> {
        ...
        const incoming = new Map<string, { name: string; description: string | null }>();
        for (const project of projects) incoming.set(fingerprint(project.name, project.description), project);

        const newProjects = [...incoming.entries()]
            .filter(([key]) => !existingByKey.has(key))
            .map(([fingerprint, project]) => ({ candidateId, fingerprint, ...project }));
        const changedProjects = [] as Array<{ id: string; candidateId: string; fingerprint: string; name: string; description: string | null }>;
        for (const [key, project] of incoming) {
            const current = existingByKey.get(key);
            if (!current || (current.name === project.name && current.description === project.description)) continue;
    ...
    private async persistCertifications(
        tx: TransactionClient,
        candidateId: string,
        certifications: ResumeParsingResult["certifications"]
    ): Promise<Pick<ResumePersistenceResult, "certificationsCreated" | "certificationsUpdated">> {
        ...
        for (const certification of certifications) incoming.set(fingerprint(certification.name), certification);

        const newCertifications = [...incoming.entries()]
            .filter(([key]) => !existingByKey.has(key))
            .map(([fingerprint, certification]) => ({ candidateId, fingerprint, ...certification }));
        const changedCertifications = [] as Array<{ id: string; candidateId: string; fingerprint: string; name: string }>;
        for (const [key, certification] of incoming) {
            const current = existingByKey.get(key);
            if (!current || current.name === certification.name) continue;
