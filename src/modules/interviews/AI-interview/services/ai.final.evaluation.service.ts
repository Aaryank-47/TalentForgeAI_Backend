...
import { AIFinalEvaluationValidator, type AIFinalEvaluationResultDto } from "../dto/ai.final.evaluation.validator.js";
import type { AIInterviewFinalEvaluationContext } from "../interfaces/ai.interview.interface.js";
...
export class AIInterviewFinalEvaluationService {
    static async generateFinalEvaluation(sessionId: string) {
        ...
        const formattedQuestions = validQuestions.map(q => {
            ...
            return {
                ...
                expectedAreas: (q.expectedAreas as string[]) || [],
                candidateAnswer: q.answer!.answerText,
                evaluation: {
                    score: ev.score,
                    evaluation: ev.feedback || "",
                    strengths: (ev.strengths as string[]) || [],
                    weaknesses: (ev.weaknesses as string[]) || []
                }
            };
        });

        const context: AIInterviewFinalEvaluationContext = {
            interview: {
        ...
        const validated: AIFinalEvaluationResultDto = AIFinalEvaluationValidator.parse(parsed);
        ...
        return AIInterviewEvaluationRepository.upsertResult({
            ...
            recommendation: validated.recommendation as AIRecommendation
        });
    }

    static async getFinalEvaluation(sessionId: string) {
    ...
    static async getFinalReport(sessionId: string) {
        const session = await InterviewSessionsRepositories.findSessionWithJobAndAIConfig(sessionId);
        if (!session) {
            throw new NotFoundError(`Interview session with ID "${sessionId}" not found.`);
        }
