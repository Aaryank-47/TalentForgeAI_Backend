...
describe("NORMAL Live 1-to-1 Interview Phase 1 Backend Suite", () => {
    ...
    test("1. Starting a valid SCHEDULED session changes status to IN_PROGRESS", async () => {
        const updated = await InterviewSessionsServices.startSession(
            company.id,
            session.id,
            recruiterUser.id
        );
    ...
    test("2. Starting an already active session is rejected", async () => {
        await expect(
            InterviewSessionsServices.startSession(
                company.id,
                session.id,
                recruiterUser.id
            )
    ...
    test("3. Candidate cannot start or end the session", async () => {
        ...
        await expect(
            InterviewSessionsServices.endSession(
                company.id,
                session.id,
                candidateUser.id
            )
    ...
    test("4. Authorized recruiter can submit scorecard/evaluation", async () => {
        const evalResult = await InterviewEvaluationServices.submitEvaluation(
            company.id,
            session.id,
            recruiterUser.id,
            {
    ...
    test("5. Candidate cannot submit recruiter evaluation", async () => {
        await expect(
            InterviewEvaluationServices.submitEvaluation(
                company.id,
                session.id,
                candidateUser.id,
                {
    ...
    test("7. Ending an IN_PROGRESS session changes status to COMPLETED", async () => {
        const completed = await InterviewSessionsServices.endSession(
            company.id,
            session.id,
            recruiterUser.id
        );
