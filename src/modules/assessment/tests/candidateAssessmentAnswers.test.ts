...
describe("Candidate Assessment Answers Get and Delete API tests", () => {
    ...
    let emptyAttempt: any;
    let job: any;
    let resume: any;
    let application: any;

    let savedMcqAnswer: any;
    let savedDsaAnswer: any;
    ...
    afterAll(async () => { 
        ...
        const attemptIds = [activeAttempt?.id, submittedAttempt?.id, cancelledAttempt?.id, expiredAttempt?.id, expiredTimerAttempt?.id, emptyAttempt?.id].filter(Boolean);
    ...
    describe("GET /api/v1/assessment-attempts/:attemptId/answers", () => {
        ...
        test("Throws ForbiddenError if attempt belongs to another candidate", async () => {
            await expect(
                AssessmentAttemptService.getAnswers(candidate2User.id, activeAttempt.id)
            ).rejects.toThrow(ForbiddenError);
        });

        test("Throws NotFoundError if attempt does not exist", async () => {
            await expect(
                AssessmentAttemptService.getAnswers(candidateUser.id, "non-existent-attempt-id")
    ...
    describe("GET /api/v1/assessment-attempts/:attemptId/answers/:questionId", () => {
        ...
        test("Throws NotFoundError if answer has not been saved yet", async () => {
            await expect(
                AssessmentAttemptService.getAnswer(candidateUser.id, emptyAttempt.id, mcqQuestion.id)
            ).rejects.toThrow(NotFoundError);
        });

        test("Throws ForbiddenError if attempt belongs to another candidate", async () => {
            await expect(
                AssessmentAttemptService.getAnswer(candidate2User.id, activeAttempt.id, mcqQuestion.id)
            ).rejects.toThrow(ForbiddenError);
        });

        test("Throws NotFoundError if attempt does not exist", async () => {
            await expect(
                AssessmentAttemptService.getAnswer(candidateUser.id, "non-existent-attempt-id", mcqQuestion.id)
            ).rejects.toThrow(NotFoundError);
        });
    });
    ...
    describe("DELETE /api/v1/assessment-attempts/:attemptId/answers/:questionId", () => {
        test("Throws ForbiddenError if attempt belongs to another candidate", async () => {
            await expect(
                AssessmentAttemptService.clearAnswer(candidate2User.id, activeAttempt.id, mcqQuestion.id)
            ).rejects.toThrow(ForbiddenError);
        });

        test("Throws ConflictError if attempt is SUBMITTED", async () => {
            await expect(
                AssessmentAttemptService.clearAnswer(candidateUser.id, submittedAttempt.id, mcqQuestion.id)
            ).rejects.toThrow(ConflictError);
        });

        test("Throws ConflictError if attempt is EXPIRED", async () => {
            await expect(
                AssessmentAttemptService.clearAnswer(candidateUser.id, expiredAttempt.id, mcqQuestion.id)
            ).rejects.toThrow(ConflictError);
        });

        test("Throws ConflictError if attempt is CANCELLED", async () => {
            await expect(
                AssessmentAttemptService.clearAnswer(candidateUser.id, cancelledAttempt.id, mcqQuestion.id)
            ).rejects.toThrow(ConflictError);
        });

        test("Throws ConflictError and sets attempt to EXPIRED if timer is expired", async () => {
            await expect(
                AssessmentAttemptService.clearAnswer(candidateUser.id, expiredTimerAttempt.id, mcqQuestion.id)
            ).rejects.toThrow(ConflictError);
        ...
        test("Throws NotFoundError if answer does not exist", async () => {
            await expect(
                AssessmentAttemptService.clearAnswer(candidateUser.id, emptyAttempt.id, mcqQuestion.id)
            ).rejects.toThrow(NotFoundError);
        });
