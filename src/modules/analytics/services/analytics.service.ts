...
export class AnalyticsService {
  ...
  static async getOverview(
    ...
    const { startDate, previousStartDate, previousEndDate, monthCount, periodLabel } =
      this.parsePeriod(periodQuery);
    ...
    const prevApps = applications.filter(
      (a) => new Date(a.appliedAt) >= previousStartDate && new Date(a.appliedAt) < startDate
    );

    const currentHiredApps = currentApps.filter(
      (a) => a.status === ApplicationStatus.HIRED && a.hiredAt
    );
    const prevHiredApps = prevApps.filter(
      (a) => a.status === ApplicationStatus.HIRED && a.hiredAt
    );

    const calcAvgDaysToHire = (apps: typeof currentHiredApps): number => {
    ...
    const prevOpenJobs = jobs.filter(
      (j) => new Date(j.createdAt) < startDate && (j.status === JobStatus.PUBLISHED || (j.closedAt && new Date(j.closedAt) >= startDate))
    ).length;
    ...
    const offerAcceptancePct =
      offeredApps.length > 0 ? Math.round((totalHires / offeredApps.length) * 100) : totalHires > 0 ? 100 : 0;
    ...
    for (const interview of interviews) {
      for (const session of interview.sessions) {
        for (const evalItem of session.evaluations) {
          totalEvaluations++;
          if (
            evalItem.recommendation === AIRecommendation.STRONG_HIRE ||
    ...
    const interviewSuccessPct =
      totalEvaluations > 0 ? Math.round((successfulEvaluations / totalEvaluations) * 100) : 0;
    ...
    const now = new Date();
    const timeToHireData: TimeToHireTrendData[] = [];
    const jobsFilledData: JobsFilledData[] = [];
    ...
    for (let i = monthCount - 1; i >= 0; i--) {
      ...
      const mHiredApps = applications.filter((a) => {
        if (a.status !== ApplicationStatus.HIRED || !a.hiredAt) return false;
      ...
      const mFilledJobs = jobs.filter((j) => {
        if (!j.closedAt && j.status !== JobStatus.FILLED) return false;
    ...
    const sourceData: SourceEffectivenessData[] = Object.entries(sourceCounts)
      .map(([name, count]) => ({
        name,
        count,
    ...
    const interviewSuccessData: InterviewSuccessData[] = [];
    const interviewTypeStats: Record<string, { total: number; success: number }> = {};
    ...
    for (const interview of interviews) {
      ...
      for (const session of interview.sessions) {
        for (const ev of session.evaluations) {
          ...
          if (
            ev.recommendation === AIRecommendation.STRONG_HIRE ||
    ...
    if (interviewSuccessData.length === 0) {
      interviewSuccessData.push(
        { name: "AI Interview", success: 0, rejected: 0 },
        { name: "Technical", success: 0, rejected: 0 }
      );
    }

    const assessmentPerfData: AssessmentPerfData[] = assessments.map((assessment) => {
      const attemptsCount = assessment.attempts.length;
      const avgScore =
        attemptsCount > 0
          ? Math.round(
              assessment.attempts.reduce((sum, att) => sum + (att.percentage ?? att.overallScore ?? 0), 0) /
                attemptsCount
            )
          : 0;
  ...
  static async getDashboard(
    ...
    // Open Jobs
    const openJobsCount = jobs.filter((j) => j.status === JobStatus.PUBLISHED).length;
    const jobsOpenedLastMonth = jobs.filter(
      (j) => j.status === JobStatus.PUBLISHED && new Date(j.createdAt) >= thirtyDaysAgo
    ).length;
    const openJobsKpi = {
      title: "Open Jobs",
      value: `${openJobsCount}`,
    ...
    const appsPrevWeek = applications.filter(
      (a) => new Date(a.appliedAt) >= fourteenDaysAgo && new Date(a.appliedAt) < sevenDaysAgo
    ).length;
    let appsWeekChange = "+0% vs last week";
    let appsWeekTrend: "up" | "down" | "warn" = "up";
    if (appsPrevWeek > 0) {
    ...
    const pendingApps = applications.filter(
      (a) => a.status === ApplicationStatus.APPLIED || a.status === ApplicationStatus.INREVIEW
    );
    const pendingToday = applications.filter(
      (a) => (a.status === ApplicationStatus.APPLIED || a.status === ApplicationStatus.INREVIEW) && new Date(a.appliedAt) >= startOfToday
    ).length;
    const pendingReviewsKpi = {
      title: "Pending Reviews",
      value: `${pendingApps.length}`,
      change: pendingToday > 0 ? `${pendingToday} need action today` : `${pendingApps.length} in pipeline`,
      trend: pendingApps.length > 0 ? ("warn" as const) : ("up" as const),
    };
    ...
    const applicationVolume: ApplicationVolumePoint[] = [];
    ...
    const calcPct = (cnt: number) => (totalApplied > 0 ? Math.round((cnt / totalApplied) * 100) : 0);

    const pipelineStages: PipelineStageDashboard[] = [
    ...
    const upcomingInterviews: UpcomingInterviewDashboard[] = upcomingSessions.map((s, i) => {
    ...
    const cancelledSessionsCount = allSessions.filter(
      (s) => s.status === InterviewSessionStatus.CANCELLED || s.status === InterviewSessionStatus.EXPIRED
    ).length;
    ...
    const interviewSummary: InterviewSummaryDashboard = {
    ...
    const recentActivity: RecentActivityDashboard[] = activityItems.slice(0, 6).map((item) => ({
      id: item.id,
      type: item.type,
      text: item.text,
    ...
    const activeJobPipelines: ActiveJobPipelineDashboard[] = activeJobs.map((j) => {
    ...
    const offerAcceptRate =
      offerCount > 0 ? `${Math.round((hiredCount / offerCount) * 100)}%` : hiredCount > 0 ? "100%" : "0%";
