...
export class SkillRepository {
     ...
     */
    public async findSkillsByNormalizedAliases(
        normalizedAliases: string[]
    ): Promise<Map<string, Skill>> {
        const resultMap = new Map<string, Skill>();
        ...
        try {
            ...
            logger.info(
                `[SkillRepository] Resolved ${resultMap.size} skill aliases out of ${uniqueKeys.length} requested lookup keys`
            );
        } catch (error: unknown) {
            logger.warn(
                `[SkillRepository] Failed to query skill aliases from database: ${
                    error instanceof Error ? error.message : "Unknown error"
                }. Gracefully continuing with fallback...`
            );
        }
     ...
     */
    public async recordSkillCandidates(
        candidates: Array<{ rawName: string; normalizedName: string }>
    ): Promise<void> {
        ...
        const validCandidates = candidates.filter(
            (c) => c && c.normalizedName && c.normalizedName.trim().length > 0 && c.rawName && c.rawName.trim().length > 0
        );
        ...
        const uniqueCandidates = Array.from(
            new Map(validCandidates.map((c) => [c.normalizedName, c])).values()
        );
        ...
        try {
            ...
            logger.info(
                `[SkillRepository] Recorded ${uniqueCandidates.length} unknown skill candidates (New: ${newCandidates.length}, Existing: ${existingCandidates.length})`
            );
        } catch (error: unknown) {
            logger.warn(
                `[SkillRepository] Failed to record ${uniqueCandidates.length} skill candidates: ${
                    error instanceof Error ? error.message : "Unknown error"
