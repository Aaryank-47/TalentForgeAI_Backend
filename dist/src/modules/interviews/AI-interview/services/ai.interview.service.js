...
import { AIGeneratedQuestionSchema, AICombinedEvaluationAndProgressionValidator } from "../dto/ai.interview.dto.js";
import { cleanJsonResponse } from "../utils/ai.interview.utils.js";
export class AIinterviewPromptService {
    static buildInitialQuestionPrompt(context) {
        const systemPrompt = `
...
`;
    ...
    static buildNextQuestionPrompt(context, history) {
        const systemPrompt = `
You are an expert AI interviewer.
...
`;
    ...
    static buildAnswerEvaluationAndProgressionPrompt(context, history, allowFollowUps) {
        const systemPrompt = `
...
`;
    ...
    static buildFinalInterviewEvaluationPrompt(context) {
        const systemPrompt = `
...
export class AIQuestionService {
    static async generateFirstQuestion(sessionId) {
    ...
    static async generateNextQuestion(sessionId, currentSequence, history) {
        const session = await InterviewSessionsRepositories.findSessionWithJobAndAIConfig(sessionId);
        if (!session) {
            throw new NotFoundError(`Interview session with ID "${sessionId}" not found.`);
        }
...
export class AIInterviewSessionService {
    static async validateAndGetCurrentQuestion(sessionId, userId) {
        const session = await InterviewSessionsRepositories.findSessionWithJobAndAIConfig(sessionId);
        if (!session) {
        ...
        const nextMainQuestion = await AIQuestionService.generateNextQuestion(sessionId, maxSequence, historyForNextPrompt);
        return {
            sessionId,
            status: "IN_PROGRESS",
            question: {
    ...
    static async submitAnswer(data) {
        ...
        try {
            progressionResult = await AIQuestionProgressionService.evaluateAndProgress(answerId, question, answerText, session);
        }
        catch (error) {
        ...
        return {
            ...
            sendOffMessage: progressionResult.completed ? progressionResult.sendOffMessage || "Thank you for completing your AI technical interview! Your responses have been successfully recorded and submitted to the hiring team for evaluation." : null,
            result: progressionResult.completed ? progressionResult.result : null
        };
    }
}
export class AIQuestionProgressionService {
    static async evaluateAndProgress(answerId, question, answerText, session) {
        const config = session.interview.aiConfiguration;
        if (!config) {
        ...
        const context = {
            ...
            expectedAreas: question.expectedAreas || []
        };
        const prompts = AIinterviewPromptService.buildAnswerEvaluationAndProgressionPrompt(context, promptHistory, allowFollowUps);
        ...
        if (decision.shouldFollowUp && decision.followUpQuestion && fullHistory.length < maxTotalQuestions) {
            ...
            const followUpQuestion = await AIInterviewQuestionsRepository.createQuestion({
                ...
                difficulty: decision.followUpQuestion.difficulty ?? question.difficulty ?? config.difficulty ?? null,
        ...
        const maxSequence = fullHistory.reduce((max, q) => Math.max(max, q.sequence), 0);
        const nextMainQuestion = await AIQuestionService.generateNextQuestion(session.id, maxSequence, historyForNextPrompt);
        return {
            evaluation: savedEvaluation,
