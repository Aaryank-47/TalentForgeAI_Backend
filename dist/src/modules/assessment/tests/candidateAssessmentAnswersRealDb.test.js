...
describe("Mode 2: Real Database Assessment Answers (Get and Clear) Integration Test", () => {
    ...
    test("Execute GET all, GET one, and DELETE answer APIs on real DB", async () => {
        ...
        // Save answer
        const savedRes = await AssessmentAttemptService.saveAnswer(candidate.user.id, attempt.id, question.id, payload);
        ...
        await expect(AssessmentAttemptService.getAnswer(candidate.user.id, attempt.id, question.id)).rejects.toThrow();
    });
});
//# sourceMappingURL=candidateAssessmentAnswersRealDb.test.js.map