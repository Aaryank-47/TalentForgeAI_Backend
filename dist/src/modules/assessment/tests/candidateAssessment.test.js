...
describe("Candidate Assessment Answer Save/Autosave API tests", () => {
    ...
    test("12. Candidate cannot access another candidate's attempt", async () => {
        ...
        await assert.rejects(AssessmentAttemptService.saveAnswer(candidate2User.id, activeAttempt.id, mcqQuestion.id, payload), ForbiddenError);
    });
    test("13. Question outside assessment is rejected", async () => {
        ...
        await assert.rejects(AssessmentAttemptService.saveAnswer(candidateUser.id, activeAttempt.id, isolatedQuestion.id, payload), ForbiddenError);
    ...
    test("14. NOT_STARTED rejected", async () => {
        ...
        await assert.rejects(AssessmentAttemptService.saveAnswer(candidateUser.id, notStartedAttempt.id, mcqQuestion.id, payload), ConflictError);
    });
    ...
    test("16. SUBMITTED rejected", async () => {
        ...
        await assert.rejects(AssessmentAttemptService.saveAnswer(candidateUser.id, submittedAttempt.id, mcqQuestion.id, payload), ConflictError);
    });
    test("17. CANCELLED rejected", async () => {
        const payload = {
            selectedOptionIds: [mcqOption1.id]
        };
        await assert.rejects(AssessmentAttemptService.saveAnswer(candidateUser.id, cancelledAttempt.id, mcqQuestion.id, payload), ConflictError);
    });
    test("18. EXPIRED rejected", async () => {
        const payload = {
            selectedOptionIds: [mcqOption1.id]
        };
        await assert.rejects(AssessmentAttemptService.saveAnswer(candidateUser.id, expiredAttempt.id, mcqQuestion.id, payload), ConflictError);
    });
    test("19. Timer expiration rejects save", async () => {
        const payload = {
            selectedOptionIds: [mcqOption1.id]
        };
        await assert.rejects(AssessmentAttemptService.saveAnswer(candidateUser.id, expiredTimerAttempt.id, mcqQuestion.id, payload), ConflictError);
    });
    test("20. Invalid MCQ option rejected", async () => {
        const payload = {
            selectedOptionIds: ["non_existent_option_id"]
        };
        await assert.rejects(AssessmentAttemptService.saveAnswer(candidateUser.id, validationAttempt.id, mcqQuestion.id, payload), ValidationError);
    });
    ...
    test("24. Missing code rejected according to validation rules", async () => {
        ...
        await assert.rejects(AssessmentAttemptService.saveAnswer(candidateUser.id, validationAttempt.id, dsaQuestion.id, payload), ValidationError);
    });
...
//# sourceMappingURL=candidateAssessment.test.js.map