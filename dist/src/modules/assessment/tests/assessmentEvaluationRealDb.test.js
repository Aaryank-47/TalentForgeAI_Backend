...
describe("Mode 2: Real Database Assessment Evaluation Integration Test", () => {
    ...
    test("Recruiter and Candidate API evaluation workflow on real DB", async () => {
        ...
        const saveRes = await AssessmentAttemptService.saveAnswer(candidateUser.id, attempt.id, question.id, { selectedOptionIds: [question.mcqDetail.options[0].id] });
        ...
        const manualRes = await AssessmentEvaluationService.evaluateQuestionManually(employerUser.id, attempt.id, question.id, 8.0, "Good choice of option.");
...
//# sourceMappingURL=assessmentEvaluationRealDb.test.js.map