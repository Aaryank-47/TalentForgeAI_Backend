...
export class AssessmentEvaluationService {
    static async startEvaluation(userId, attemptId) {
    ...
    static async runOrchestrator(attemptId) {
        try {
    ...
    static async getEvaluationStatus(userId, role, attemptId) {
        const attempt = await AssessmentEvaluationRepository.findAttemptById(attemptId);
        if (!attempt) {
            throw new NotFoundError("Assessment attempt not found.");
        }
    ...
    static async runCode(userId, attemptId, questionId, code, languageId) {
        const attempt = await AssessmentEvaluationRepository.findAttemptById(attemptId);
        if (!attempt) {
            throw new NotFoundError("Assessment attempt not found.");
        }
    ...
    static async evaluateQuestionManually(userId, attemptId, questionId, score, feedback) {
        const attempt = await AssessmentEvaluationRepository.findAttemptById(attemptId);
        if (!attempt) {
            throw new NotFoundError("Assessment attempt not found.");
        }
    ...
    static async getFinalResult(userId, role, attemptId) {
        const attempt = await AssessmentEvaluationRepository.findAttemptById(attemptId);
        if (!attempt) {
            throw new NotFoundError("Assessment attempt not found.");
        }
...
export class MCQEvaluationService {
    static async evaluateAttempt(attemptId) {
        const attempt = await AssessmentEvaluationRepository.findAttemptWithAnswersAndQuestions(attemptId);
        if (!attempt)
            return { totalQuestions: 0, answeredQuestions: 0, correctAnswers: 0, score: 0, totalMarks: 0 };
        ...
        for (const section of attempt.assessment.sections) {
            for (const item of section.items) {
                ...
                if (answer) {
                    ...
                    score += questionScore;
                    await AssessmentEvaluationRepository.updateQuestionEvaluation(attemptId, question.id, questionScore, isCorrect ? "Correct answer" : "Incorrect answer", isCorrect);
                }
            }
        }
        return {
...
export class DSAEvaluationService {
    static async evaluateAttempt(attemptId) {
        const attempt = await AssessmentEvaluationRepository.findAttemptWithAnswersAndQuestions(attemptId);
        if (!attempt)
            return { totalQuestions: 0, passedQuestions: 0, score: 0, totalMarks: 0 };
        let totalQuestions = 0;
            ...
            for (const item of section.items) {
                ...
                if (answer && answer.codeResponse) {
                    ...
                    if (typeof answer.codeResponse === 'string') {
                        ...
                        code = answer.codeResponse.code || JSON.stringify(answer.codeResponse);
                    }
                    ...
                    score += questionScore;
                    await AssessmentEvaluationRepository.updateQuestionEvaluation(attemptId, question.id, questionScore, isPassed ? "All test cases passed." : "Test cases failed.", isPassed);
                }
            }
        }
        return {
...
export class AssessmentResultService {
    static async calculateFinalResult(attemptId) {
        const attempt = await AssessmentEvaluationRepository.findAttemptWithAnswersAndQuestions(attemptId);
        if (!attempt) {
            throw new NotFoundError("Assessment attempt not found.");
        }
        ...
        await AssessmentEvaluationRepository.updateAssessmentAttemptResult(attemptId, overallScore, percentage, passed, EvaluationStatus.COMPLETED);
...
//# sourceMappingURL=assessmentEvaluation.service.js.map