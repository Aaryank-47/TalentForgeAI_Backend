...
export class JobAssessmentService {
    ...
    static async getJobAssessments(jobId) {
        const job = await JobAssessmentRepository.findJobById(jobId);
        if (!job) {
            throw new NotFoundError("Job not found");
        }
    ...
    static async updateJobAssessment(jobId, dto, user) {
        const job = await JobAssessmentRepository.findJobById(jobId);
        if (!job) {
            throw new NotFoundError("Job not found");
        }
    ...
    static async removeJobAssessment(jobAssessmentId) {
    ...
    static async reorderJobAssessments(dto) {
    ...
    static async createAssessmentInvitation(applicationId, dto, idempotencyKey) {
        if (idempotencyKey) {
        ...
        const activeInvite = application.assessmentInvitations.find((inv) => inv.assessmentId === assessmentId && inv.status === InvitationStatus.PENDING);
        if (activeInvite) {
            throw new ConflictError("An active assessment invitation already exists.");
        }
        const completedInvite = application.assessmentInvitations.find((inv) => inv.assessmentId === assessmentId && inv.status === InvitationStatus.SUBMITTED);
        if (completedInvite) {
        ...
        if (dto.sendEmail && application.candidate.user.email) {
            ...
            const template = emailTemplates.assessmentInvitationTemplate(application.candidate.fullName, assessment.title, expiresAtDate.toUTCString(), inviteLink);
            const emailTemplate = {
    ...
    static async getCandidateMyInvitations(userId) {
    ...
    static async getAssessmentInvitation(applicationId) {
    ...
    static async resendInvitation(id) {
        ...
        if (invitation.application.candidate.user.email) {
            ...
            const template = emailTemplates.assessmentInvitationTemplate(invitation.application.candidate.fullName, invitation.assessment.title, new Date(invitation.expiresAt).toUTCString(), inviteLink);
            const emailTemplate = {
    ...
    static async cancelInvitation(id) {
        const invitation = await JobAssessmentRepository.findInvitationById(id);
        if (!invitation) {
            throw new NotFoundError("Invitation not found.");
        }
    ...
    static async expireInvitation(id) {
        const invitation = await JobAssessmentRepository.findInvitationById(id);
        if (!invitation) {
            throw new NotFoundError("Invitation not found.");
        }
...
//# sourceMappingURL=assessmentAssignment.service.js.map