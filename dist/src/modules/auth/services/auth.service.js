...
export class AuthService {
    ...
    static async logoutAllDevicesByEmail(payload) {
        const user = await AuthRepository.findLoginUserByEmail(payload.email);
        if (!user) {
            throw new NotFoundError("User not found.");
        }
    ...
    static async changePassword(userId, oldPassword, newPassword) {
        ...
        const hashedPassword = await bcrypt.hash(newPassword, AUTH_CONSTANTS.PASSWORD_SALT_ROUNDS);
    ...
    static async forgotPassword(email) {
        ...
        const hashOtp = await bcrypt.hash(otp, AUTH_CONSTANTS.OTP_HASH_SALT_ROUNDS);
        await AuthRepository.saveOTP(user.id, hashOtp, new Date(Date.now() + AUTH_CONSTANTS.OTP_EXPIRY));
    ...
    static async verifyOtp(payload) {
        ...
        await AuthRepository.saveResetPasswordToken(user.id, resetPasswordToken, getResetPasswordTokenExpiresAt(resetPasswordToken));
    ...
    static async resetPassword(resetPasswordToken, newPassword) {
        ...
        const hanshNewPassword = await bcrypt.hash(newPassword, AUTH_CONSTANTS.PASSWORD_SALT_ROUNDS);
    ...
    static async registerEmployer(payload) {
        const existingUser = await AuthRepository.findUserByEmail(payload.email);
        if (existingUser) {
            throw new ConflictError("An account with this email already exists.");
        }
    ...
    static async registerCompanyOwner(payload) {
        const existingUser = await AuthRepository.findUserByEmail(payload.email);
        if (existingUser) {
            throw new ConflictError("An account with this email already exists.");
        }
    ...
    static async resendVerificationEmail(payload) {
        const user = await AuthRepository.findUserByEmail(payload.email);
        if (!user) {
            throw new NotFoundError(MESSAGE.USER_NOT_FOUND);
        }
        ...
        await AuthRepository.saveOTP(user.id, hashedOtp, new Date(Date.now() + AUTH_CONSTANTS.EMAIL_VERIFICATION_OTP_EXPIRY));
