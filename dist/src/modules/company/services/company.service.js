...
export class CompanyService {
    ...
    static async sendInvitation(companyId, inviterId, inviteeEmail, role) {
        ...
        if (invitee) {
            ...
            if (existingMembership) {
                ...
                const updatedMember = await CompanyRepository.updateMembership(existingMembership.id, {
                    role,
    ...
    static async updateCompanyMemberRole(companyId, userId, role) {
        ...
        if (company && member.user?.email) {
            const memberName = member.user.employer?.fullName ||
                member.user.candidate?.fullName ||
                "Team Member";
            const emailTemplate = emailTemplates.memberRoleUpdatedTemplate(company.companyName, memberName, role, oldRole);
    ...
    static async cancelInvitation(invitationId, userId) {
        ...
        try {
            ...
            if (company && inviteeUser && inviteeUser.email) {
                ...
                const recipientName = inviteeProfile?.profile?.fullName ?? (inviteeUser.email ? inviteeUser.email.split("@")[0] : "Candidate");
                const emailTemplate = emailTemplates.invitationCancelledTemplate(company.companyName, recipientName, invitation.role);
                EmailService.sendEmail({
