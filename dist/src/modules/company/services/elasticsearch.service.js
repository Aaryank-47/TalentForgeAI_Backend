...
import { ES_INDICES, ES_COMPANY_MAPPINGS, ES_COMPANY_BOOSTS, ES_MAX_RESULT_WINDOW, } from "../../../common/constants/elasticsearch.constants.js";
...
export class ElasticsearchService {
     ...
     */
    static async syncExistingCompanies() {
        try {
     ...
     */
    static async indexCompany(company) {
        try {
            ...
            logger.error({ err: error, companyId: company.id }, "[Elasticsearch] Failed to index company.");
        }
    }
    /**
    ...
    static async removeCompany(companyId) {
        try {
            ...
            if (error?.meta?.statusCode !== 404) {
                logger.error({ err: error, companyId }, "[Elasticsearch] Failed to remove company.");
            }
        }
    }
    /**
