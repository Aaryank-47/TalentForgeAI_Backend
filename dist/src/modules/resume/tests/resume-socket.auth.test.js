    ...
    describe("Authorization & Room Subscription", () => {
        ...
        test("allows candidate to subscribe to their own resume and receive real-time stage events", async () => {
            ...
            const completedPromise = new Promise((resolve) => {
                clientA.on(RESUME_SOCKET_EVENTS.STAGE_CHANGE, (payload) => {
                    stageEvents.push(payload);
                });
                clientA.on(RESUME_SOCKET_EVENTS.COMPLETED, (payload) => {
                    resolve(payload);
                });
            });
        ...
        test("publishes final failure event when resume fails permanently", async () => {
            ...
            const failedPromise = new Promise((resolve) => {
                clientA.on(RESUME_SOCKET_EVENTS.FAILED, (data) => resolve(data));
            });
        ...
        test("resilience: publisher failure never throws or breaks execution", async () => {
            ...
            await expect(ResumeProgressPublisher.publishStageChange("AI_PARSING", {
                jobId: "job-error",
                resumeId: "resume-err",
                candidateId: "candidate-err"
            })).resolves.toBeUndefined();
            await expect(ResumeProgressPublisher.publishCompleted({
                jobId: "job-error",
                resumeId: "resume-err",
                candidateId: "candidate-err"
            })).resolves.toBeUndefined();
            await expect(ResumeProgressPublisher.publishFinalFailure({ jobId: "job-error", resumeId: "resume-err", candidateId: "candidate-err" }, "Some error")).resolves.toBeUndefined();
            if (originalNsp) {
...
//# sourceMappingURL=resume-socket.auth.test.js.map