...
describe("Resume Processing - Manual Retry Mechanism", () => {
    ...
    describe("addResumeProcessingJob - ID Strategy", () => {
        it("Requirement A: Initial processing uses deterministic jobId resume-processing-${resumeId}", async () => {
            ...
            const mockJob = { id: `resume-processing-${resumeId}`, getState: jest.fn().mockResolvedValue("waiting") };
            ...
            expect(addSpy).toHaveBeenCalledWith("process-resume", expect.any(Object), expect.objectContaining({
        ...
        it("Requirement B & C: Manual retry accepts a unique retry jobId", async () => {
            ...
            const mockJob = { id: customRetryJobId, getState: jest.fn().mockResolvedValue("waiting") };
            const addSpy = jest.spyOn(queue, "add").mockResolvedValue(mockJob);
            const job = await addResumeProcessingJob({
                candidateId,
                resumeId,
    ...
    describe("CandidateService.retryResumeProcessing", () => {
        it("Requirement H: Rejects retry if resume is already COMPLETED", async () => {
            ...
            await expect(CandidateService.retryResumeProcessing(resumeId, candidateId)).rejects.toThrow(ConflictError);
            await expect(CandidateService.retryResumeProcessing(resumeId, candidateId)).rejects.toThrow("Resume has already been successfully processed");
        });
        it("Requirement I: Rejects retry if resume is currently PROCESSING", async () => {
            ...
            await expect(CandidateService.retryResumeProcessing(resumeId, candidateId)).rejects.toThrow(ConflictError);
            await expect(CandidateService.retryResumeProcessing(resumeId, candidateId)).rejects.toThrow("Resume is currently being processed");
        });
        it("Requirement D, E & K: Successfully retries FAILED resume, creates unique job, resets DB to QUEUED, and notifies Socket.IO", async () => {
            ...
            const publishSpy = jest.spyOn(ResumeProgressPublisher, "publishStageChange").mockResolvedValue(undefined);
        ...
        it("Requirement G: Multiple manual retries calculate incremented attempt counters (retry-1, retry-2)", async () => {
            ...
            jest.spyOn(queue, "getJob").mockImplementation(async (jobId) => {
        ...
        it("Requirement 6: Reverts DB status to FAILED if queue enqueueing throws an error", async () => {
            ...
            const revertSpy = jest.spyOn(prisma.resume, "update").mockResolvedValue({});
            await expect(CandidateService.retryResumeProcessing(resumeId, candidateId)).rejects.toThrow("Redis connection timed out");
    ...
    describe("Worker Execution of Manual Retry Job (QUEUED -> PROCESSING -> COMPLETED/FAILED)", () => {
        it("Requirement D & E: Worker receives the manual retry job, updates DB to PROCESSING, and completes pipeline", async () => {
            ...
            const worker = new (await import("../queues/resume-processing.worker.js")).ResumeProcessingWorker(mockPipeline, jest.fn());
            const prismaFindSpy = jest.spyOn(prisma.resume, "findUnique").mockResolvedValue({
                id: resumeId,
                parsingStatus: "QUEUED"
            });
            const prismaUpdateSpy = jest.spyOn(prisma.resume, "update").mockResolvedValue({});
            const publishCompletedSpy = jest.spyOn(ResumeProgressPublisher, "publishCompleted").mockResolvedValue(undefined);
            const mockJob = {
                id: `resume-processing-${resumeId}-retry-1`,
                data: {
                    candidateId,
            ...
            expect(mockPipeline.execute).toHaveBeenCalledWith(mockJob.data, mockJob.id, expect.any(Function));
            ...
            expect(publishCompletedSpy).toHaveBeenCalledWith(expect.objectContaining({
                jobId: mockJob.id,
                resumeId
            }));
        });
...
//# sourceMappingURL=resume-retry.test.js.map