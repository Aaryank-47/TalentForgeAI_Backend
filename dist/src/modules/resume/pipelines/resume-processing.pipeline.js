...
export class ResumeProcessingPipeline {
    ...
    fileFetcher;
    constructor(documentExtractorService = new DocumentExtractorService(), resumeParserService = new ResumeParserService(), resumeNormalizationService = new ResumeNormalizationService(), resumePersistenceService = new ResumePersistenceService(), fileFetcher = fetchFileBufferFromUrl) {
     ...
     */
    async execute(jobData, jobId, onStageChange) {
        const { candidateId, resumeId, fileReference, mimeType } = jobData;
        const startTime = performance.now();
        const baseMeta = { jobId, resumeId, candidateId };
        ...
        if (normalizedMimeType === RESUME_MIME_TYPES.DOCX) {
            ...
            await this.notifyStage("EXTRACTION", {
                ...baseMeta,
                mode: "DIRECT"
            }, onStageChange);
            ...
            await this.notifyStage("AI_PARSING", {
                ...baseMeta,
                mode: "DIRECT"
            }, onStageChange);
            try {
                parsedData = await this.resumeParserService.parseResumeDocument(documentBuffer, normalizedMimeType);
            }
            catch (error) {
                ...
                await this.notifyStage("EXTRACTION", { ...baseMeta, mode: "FALLBACK", reason: fallbackReason }, onStageChange);
                const extractionResult = await this.documentExtractorService.extractDocument(documentBuffer, normalizedMimeType);
        ...
        await this.notifyStage("NORMALIZATION", baseMeta, onStageChange);
        const normalizedData = await this.resumeNormalizationService.normalizeResumeData(parsedData);
        // Stage 5: PERSISTENCE
        await this.notifyStage("PERSISTENCE", baseMeta, onStageChange);
        const persistenceResult = await this.resumePersistenceService.persistResumeData(candidateId, normalizedData);
    ...
    async notifyStage(stage, meta, onStageChange) {
        logger.info({
        ...
        if (onStageChange) {
            try {
                ...
                logger.warn({ err, stage, resumeId: meta.resumeId }, `[ResumeProcessingPipeline] Stage change handler failed for stage "${stage}"`);
            }
        }
    }
}
//# sourceMappingURL=resume-processing.pipeline.js.map