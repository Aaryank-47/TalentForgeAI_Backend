...
export class ResumeProcessingWorker {
    ...
    pipeline;
    stageChangeHandler;
    constructor(pipeline = new ResumeProcessingPipeline(), stageChangeHandler = async (stage, meta) => {
    ...
    start() {
        ...
        logger.info({ concurrency, queueName: RESUME_PROCESSING_QUEUE_NAME }, "[ResumeProcessingWorker] Initializing worker...");
        this.worker = new Worker(RESUME_PROCESSING_QUEUE_NAME, async (job) => {
    ...
    async processJob(job) {
        ...
        try {
            ...
            if (existingResume.parsingStatus === "COMPLETED") {
                logger.warn({ jobId, resumeId, candidateId }, `[ResumeProcessingWorker] Resume "${resumeId}" is already COMPLETED. Skipping redundant processing.`);
            ...
            const { normalizedData, persistenceResult, durationMs } = await this.pipeline.execute(job.data, jobId, this.stageChangeHandler);
            ...
            if (isPermanent || isFinalAttempt) {
                ...
                await ResumeProgressPublisher.publishFinalFailure({ jobId, resumeId, candidateId }, errorMessage);
            }
            else {
    ...
    isPermanentError(error) {
        ...
        if (error instanceof EmptyDocumentTextError)
            return true;
        if (error instanceof ScannedPdfDetectedError)
            return true;
     ...
     */
    async updateResumeStatus(resumeId, parsingStatus, additionalData = {}) {
        try {
            ...
            logger.error({ err: error, resumeId, parsingStatus }, `[ResumeProcessingWorker] Failed to update resume parsing status to "${parsingStatus}"`);
            throw error;
        }
    }
    /**
...
let resumeWorkerInstance = null;
export function getResumeProcessingWorker() {
    if (!resumeWorkerInstance) {
        resumeWorkerInstance = new ResumeProcessingWorker();
    }
    return resumeWorkerInstance;
}
//# sourceMappingURL=resume-processing.worker.js.map