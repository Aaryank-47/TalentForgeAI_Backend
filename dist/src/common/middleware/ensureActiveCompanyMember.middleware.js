...
export const ensureActiveCompanyMember = async (req, res, next) => {
    try {
        const user = req.user;
        if (!user) {
        ...
        let membership = await CompanyRepository.findMemberByUserAndCompany(user.id, companyId);
        if (!membership) {
...
//# sourceMappingURL=ensureActiveCompanyMember.middleware.js.map